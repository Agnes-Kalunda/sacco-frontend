...
import { faMobileAlt, faBook, faHandshake, faMoneyBillWave, faChartLine, faUsers } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <div className="services">
      <Container className="p-4">
        <h2 className='service' style={{ marginBottom: '10px' }}><strong>Our Services</strong></h2>
        <hr style={{ marginBottom: '20px' }} />
        <Row xs={1} md={2} lg={3} className="g-4">
          {cardsData.map((card) => (
            <Col key={card.id}>
              <Card className='h-100 no-border-card'>
                <FontAwesomeIcon icon={card.icon} size="3x" style={{ margin: '20px auto' }} />
                <Card.Body>
